...
var now = Date.now(),
	hrtimeBase = process.hrtime();
var hrtimeOffset = [
	Math.ceil((now / 1000) - hrtimeBase[0]),
	Math.ceil(1000 * (now % 1000) - (hrtimeBase[1]/1000)) % 1000000
];
...
var ObjectId = module.exports = function(bytes) {
    ...
    self.init = function(bytes) {
        if (Buffer.isBuffer(bytes)) {
            ...
            var hrtimestamp = process.hrtime(),
				timestamp = [
					hrtimestamp[0] + hrtimeOffset[0],
					hrtimestamp[1] + hrtimeOffset[1]
				];
